...
# Start an agent with a custom command
agent run -c 'gemini' my-feature
...
3. The agent command (defaults to `gemini --yolo`) is automatically started in the tmux session.
...
- **Agent Command**: By default, the script runs `gemini --yolo`. This can be overridden using the `-c/--command` flag.
